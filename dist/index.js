...
function Saab() {
    ...
    function addHandler(method, path, handler) {
        const pathMatcher = (0, path_to_regexp_1.match)(path);
        handlers.push({
            method,
            path,
            pathMatcher,
    ...
    async function listen(wv, js = "") {
        ...
        if (!handler)
            return res.error(new Error("No matching route handler"));
        try {
